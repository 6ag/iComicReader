 ...
 *  轮播按钮点击事件
 */
- (void)didTappedImageView:(UITapGestureRecognizer *)tap {
    
    HeaderModel *model = self.modelArray[_index];
    
    if (model.type.integerValue == 2) {
        
        // 列表
        JFComicSectionsListViewController *controller = [[JFComicSectionsListViewController alloc]init];
        controller.bookID = model.value;
...
#pragma mark 启动计时器
- (void)startTimer{
    _timer = [NSTimer timerWithTimeInterval:4.0 target:self selector:@selector(nextImageViewForScrollView) userInfo:nil repeats:YES];
...
- (void)scrollViewAddContentView{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(didTappedImageView:)];
